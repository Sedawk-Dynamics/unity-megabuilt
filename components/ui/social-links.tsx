...
/** Inline brand SVGs (24×24, currentColor) so we don't depend on lucide brand icons. */
    ...
    <div className={cn('flex flex-wrap items-center gap-2.5', className)}>
      {socials.map((s) => (
        <a
          ...
          aria-label={s.label}
          title={s.label}
          className="flex size-10 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-colors duration-300 hover:border-gold/50 hover:bg-gold/10 hover:text-gold"
        >
          {icons[s.key]}
        </a>
      ))}
